...
def p_assignment_word(p):
    ...
    | IDENTIFIER EQUALS arithmetic_expression"""
    p[0] = {
        "type": "assignment_word",
        "name": p[1],
        "value": p[3],
    }
    memory[p[1]] = {"type": "assignment", "value": p[3]}
    # pprint.pp(f"memory: {memory}")
...
def p_identifier_value(p):
    """identifier_value : DOLLAR IDENTIFIER"""
    if memory[p[2]]:
        p[0] = memory[p[2]]["value"]
    else:
        p[0] = None
...
def p_else_command(p):
    """else_command : ELSE SPACE command_list"""
    p[0] = {
        "type": "else_command",
        "command": p[3],
    }
...
def p_elif_command(p):
    """elif_command : ELIF SPACE condition_chain SEMICOLON SPACE THEN SPACE command_list"""
    p[0] = {
        "type": "elif_command",
        "condition": p[3],
        "then_command": p[8],
...
def p_if_command(p):
    ...
    | IF SPACE condition_chain SEMICOLON SPACE THEN SPACE command_list SEMICOLON SPACE else_command SEMICOLON SPACE FI
    | IF SPACE condition_chain SEMICOLON SPACE THEN SPACE command_list SEMICOLON SPACE elif_command SEMICOLON SPACE FI
    | IF SPACE condition_chain SEMICOLON SPACE THEN SPACE command_list SEMICOLON SPACE elif_command SEMICOLON SPACE else_command SEMICOLON SPACE FI"""
    if len(p) == 12:
        p[0] = {
            "type": "if_command",
            "condition": p[3],
        ...
        p[0] = {
            "type": "if_else_elif_command"
            if p[11]["type"] == "elif_command"
            else "if_else_command",
            "condition": p[3],
            "then_command": p[8],
        }
        # "else_command": p[7],
...
def p_while_command(p):
    """while_command : WHILE SPACE condition_chain SEMICOLON SPACE DO SPACE command_list SEMICOLON SPACE DONE"""
    p[0] = {
        "type": "while_command",
        "condition": p[3],
        "command": p[8],
...
def p_arithmetic_expression(p):
    ...
    p[0] = {"type": "arithmetic_expression", "value": p[2]}
...
def p_factor(p):
    ...
    if len(p) == 2:
        ...
        p[0] = {"type": "unary_operation", "operator": p[1], "operand": p[2]}
    else:
        p[0] = p[2]
...
def get_commands(file: str) -> list[str]:
    return list(map(CLEAN_LINE, filter(IGNORE_LINE, open(file, "r").readlines()[1:])))
